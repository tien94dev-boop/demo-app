...
export default function Home() {
  ...
  const learningProcesses = {
    ...
    items: [
      ...
      },
      {
        ...
        des: `
        – Hoàn thành các môn học trước thời hạn
        – Được trao Giải thưởng Thành tích Học tập
  ...
  const interest = {
    ...
    des: `
    – Trượt ván
    – Chơi guitar điện và trống
    – Đạp xe đường núi
    – Chạy cự ly
    – Viết thơ và truyện viễn tưởng
  ...
  return (
    <div className="grid grid-cols-12 gap-4 bg-gray-100">
      <div className="col-span-4 bg-gray-600 text-white p-8 ">
        <div className="rounded-full overflow-hidden w-66 h-66 mt-10 mb-5">
          <Image
            src="/profile.png"
            alt="profile"
            width={100}
        ...
        <p className="text-3xl pb-5 font-bold">TIẾN PROFILE</p>
        <p className="pb-5 text-2xl">Quản lý cộng đồng</p>
        <div className="pb-10 flex flex-col gap-12">
          {features.map((feature, index) => {
            return <div key={index} className="">
              <p className="uppercase text-xl font-bold pb-4">{feature.title}</p>
              <p className="text-lg" dangerouslySetInnerHTML={{ __html: feature.description }} />
            </div>
          })}
        </div>
      </div>
